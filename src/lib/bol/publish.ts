...
import type { GeneratedArticle, BolProductRow } from './types';
...
/**
 * Publish a bol.com article to Postgres (and optionally Astro DB).
 * Links the article to the source product(s).
 */
export async function publishBolArticle(
  ...
  for (const productId of productIds) {
    await query(
      `UPDATE bol_products SET article_id = $1, updated_at = NOW() WHERE id = $2`,
      [articleId, productId]
    );
  }
