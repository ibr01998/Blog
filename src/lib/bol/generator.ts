/**
 * Bol.com Article Generator
 * Standalone AI article generator for bol.com product reviews and roundups.
 ...
 * bol.com system independent from the crypto editorial pipeline.
 */

import { generateText } from 'ai';
import { createAnthropic } from '@ai-sdk/anthropic';
import type { BolProductRow, GeneratedArticle } from './types';
...
function buildSingleProductPrompt(product: BolProductRow, language: 'nl' | 'en'): { system: string; user: string } {
...
- Gebruik contextuele ankertekst: "Bekijk op bol.com", "Bekijk de huidige prijs op bol.com"
...
- Use contextual anchor text: "Check on bol.com", "See current price on bol.com"
...
primary_keyword: [main search keyword]`;
  ...
  const affiliateUrl = `/go/bol-${product.ean || slugify(product.title)}`;
...
LEVERING: ${product.delivery_label || 'Standaard'}
KENMERKEN:
...
DELIVERY: ${product.delivery_label || 'Standard'}
FEATURES:
...
function buildMultiProductPrompt(products: BolProductRow[], language: 'nl' | 'en'): { system: string; user: string } {
  const isNl = language === 'nl';
...
- Contextuele ankertekst: "Bekijk [Product] op bol.com"
...
- Contextual anchor text: "Check [Product] on bol.com"
...
primary_keyword: [main search keyword]`;
  ...
  const productBlocks = products.map((p, i) => {
    ...
    const affiliateUrl = `/go/bol-${p.ean || slugify(p.title)}`;
    return `PRODUCT ${i + 1}:
- Naam: ${p.title}
...
- Levering: ${p.delivery_label || 'Standaard'}
...
function parseGeneratedArticle(text: string, language: 'nl' | 'en', heroImage: string): GeneratedArticle {
  ...
  const title = titleMatch?.[1] ?? meta.meta_title ?? 'Bol.com Product Review';
...
/**
 * Generate a bol.com product review or comparison article.
 * Single product → review article.
 * Multiple products → comparison/roundup article.
 */
export async function generateBolArticle(
  products: BolProductRow[],
  language: 'nl' | 'en' = 'nl'
  ...
  const result = await withTimeout(
    ...
    TIMEOUT_MS,
    `generateBolArticle (${products.length} products)`
  );
